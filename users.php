...
<div id="content">
  ...
  <form name="frmSearchUser" method="post" id="frmSearchUser" action="<?PHP echo $_SERVER['PHP_SELF']; ?>">
    <div class="displayPane">
      ...
      <div>
        <input name="txtSearch" id="txtSearch" placeholder="Search User" style="width: calc(100% - 115px)"
    ...
    <table class="table" style="width: calc(100% - 15px);border: #cccccc solid 1px">
      ...
      if($row_cnt >= 1) {
        while ($row = $result->fetch_assoc()) {
          ...
          ?>
      <tr class="hl">
        <td onclick="loadUser(<?PHP echo $sid; ?>,'Edit')" style="cursor: pointer;border-left: #cccccc solid 1px">
          <?PHP echo $sid; ?>
        </td>
        <td onclick="loadUser(<?PHP echo $sid; ?>,'Edit')" style="cursor: pointer;border-left: #cccccc solid 1px">
