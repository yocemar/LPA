...
<div id="content">
	...
	<form name="frmSearchClient" method="post" id="frmSearchClient" action="<?PHP echo $_SERVER['PHP_SELF']; ?>">
		<div class="displayPane">
			...
			<div>
				<input name="txtSearch" id="txtSearch" placeholder="Search clients" style="width: calc(100% - 115px)"
					value="<?PHP echo $txtSearch; ?>">
				<button class="btn btn-primary" type="button" id="btnSearch">Search</button>
				<?PHP
						if($isAdmin) {
					?>
				<button class="btn btn-primary" type="button" id="btnAddRec">Add</button>
				<?PHP } ?>
			</div>
		</div>
	...
	<?PHP
			if(true /* $action == "listClients" */) {
		?>
	<div>
		<table class="table" class="tableS">
			<thead class="table-dark">
				<tr class="trhtableS">
					...
					<td class="tdtableS" style="width: 100px; text-align: center"><b>Client status</b></td>
				</tr>
			</thead>
			<?PHP
				openDB();
			...
			?>
			<tr class="hl" <?PHP if($isAdmin) { ?>
				onclick="loadInvoice(
				<?PHP echo $clientid; ?>,'Edit')" style="cursor: pointer"
				<?PHP } ?>
				>
				<td class="tdtableS">
					<?PHP echo $clientid; ?>
			...
			<tr>
				<td colspan="4" class="noRecordsFound">
					No records found for: <b>
						<?PHP echo $txtSearch; ?></b>
				</td>
			</tr>
			<?PHP } ?>
		</table>
